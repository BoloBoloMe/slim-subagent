...
- 状态: 当前有效
- 约束性: 必须遵守
- 内容: 替代 D002. D002 的比率分体系 (相对基准打分, 允许 >1/<1, updatedAt + note 依据, 手工维护 + D009 派生例外) 全部保留, 唯一修正: 基准 `opencode-go/deepseek-v4-flash` 经 bootstrap 调研证实为纯文本模型 (来源: NVIDIA NIM 条目, 官方定价页无视觉特性), multimodal 维度 = null, "基准全维 = 1" 在该维不成立. 处理: multimodal 维度放弃比率语义, 改用绝对能力档 (1=可用, 1.5+=强), 仅用于 vision 画像内相对比较; 评分表头已注明. 否决"换多模态模型作基准": 会失去价格/速度维度的锚点 (基准选择正是看中 flash 的效率型定位).
- 实际影响: `~/.pi/agent/slim-subagent/llm-scores.md` 表头说明与基准行 (bootstrap 时已按本决策落地)
- 需要调整: 无

### D018 初始评分 (bootstrap 首批结果)
- 状态: 当前有效
- 约束性: 可调整
- 内容: 2026-08-19 bootstrap 产生 scoped 3 模型六维分 (price 派生), 用户已检查并认可. 弱依据条目三条, 后续按 D010 事件驱动修订: (1) kimi-for-coding/knowledge=0.9 用父族 K2.6 proxy; (2) k3/speed=0.5 与 kimi-for-coding/speed=1.2 来源第三方实测 (AA/OpenRouter) 非官方; (3) k3/coding=1.3 靠 AA 综合排名推算 (中置信). 派生价格分现算验证: 基准 unit $0.33 = 1, kimi-for-coding ≈ 0.19 (unit $1.71), k3 ≈ 0.06 (unit $6.00).
- 依赖事实: F002, F003
...
- 状态: 当前有效
- 约束性: 必须遵守
- 内容: 替代 D004. 用户裁定: 未评分 = 未审查 = 殿后. 未收录进评分表的 scoped 模型排序恒在最后 (带 [未评分] 标注), 不参与分数竞争; 但不是禁用 — 必需维度过滤后仅剩它时仍会被选. D004 的顾虑 (新模型永无试用机会) 不成立: 新模型由用户手动加进 scoped, 想试就会顺手打分. 应用实例: k3 因成本过高被用户移出评分表与 enabledModels (2026-08-20); 若日后重进 scoped, 未评分殿后自动生效.
- 实际影响: score.py 排序键; llm-scores.yaml 与 settings.json enabledModels 移除 k3
